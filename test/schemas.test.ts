...
test("validateCollection throws a descriptive error", () => {
    expect(() => validateCollection([])).toThrowError(`\
...
test("validateCollection includes input description in error", () => {
    expect(() =>
        validateCollection([], { inputDescription: "/some/collection.json" })
    ).toThrow(`\
...
test("validateCollection omits schema path of error when verbose is disabled", () => {
    expect(() =>
        validateCollection([], {
            inputDescription: "/some/collection.json",
            verbose: false,
        })
    ).toThrow(
        "/some/collection.json does not match the collection schema: collection must be object"
    );
});
