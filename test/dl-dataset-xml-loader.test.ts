        ...
        use: [
            {
                loader: path.resolve(
                    __dirname,
                    "../src/loaders/dl-dataset-xml-loader.ts"
                ),
            },
        ],
    },
...
test("dl-dataset-xml-loader", async () => {
    const stats = await compiler(
        "./data/example.dl-dataset.xml",
        xmlLoaderRules
    );

    expect(
        JSON.parse(getModuleSource("./data/example.dl-dataset.xml", stats))
    ).toEqual({
        "@type":
...
test("dl-dataset-xml-loader rejects invalid input", async () => {
    await expect(
        compiler("./data/invalid_bad_collection.dl-dataset.xml", xmlLoaderRules)
