...
test("collection references are resolved", async () => {
    const stats = await compiler(
        "./data/collections/kitchen-sink.collection.json",
        jsonLoaderRules
    );
    ...
    expect(kitchenSink["@type"]).toEqual(
        "https://schemas.cudl.lib.cam.ac.uk/package/v1/collection.json"
    );

    // References are replaced with paths to packaged content.
    expect(kitchenSink.items[0]).toEqual({
        "@id": "bundled/data/collections/referenced-modules/mock.item.json",
    });

    expect(
        fp.pipe(
            fp.map((mod: webpack.StatsModule) => ({
                name: mod.name,
                source: mod.source,
            })),
            fp.sortBy(["name"])
