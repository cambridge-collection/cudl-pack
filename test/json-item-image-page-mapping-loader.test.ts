...
const rules: webpack.RuleSetRule[] = [{
    ...
    test: /\.json$/,
    use: {
        loader: path.resolve(__dirname, '../src/loaders/item-image-page-mapping-loader.ts'),
        options: {
...
test('test loading CSV file and generating page mapped item JSON', async () => {
    ...
    const dataOutput: string = await readPathAsString('./data/item/image-mapping/item-with-pagination-inserted.json');
    const itemOut: Item = parseItemJson(generateItemJson(JSON.parse(dataOutput)));
    expect(itemOut).toEqual(JSON.parse(dataOutput));

    // validate JSON generated against expected value.
    expect(JSON.parse(getModuleSource('./data/item/image-mapping/item-with-linked-pagination.json', stats)))
...
test('test missing option imageServerPath', async () => {
    ...
    await expect(compiler('./data/item/image-mapping/item-with-linked-pagination.json', invalidRules)).rejects
        .toThrowError(/You need to set the imageServerPath and imageType parameters in your loader options./);
});
...
test('test invalid CSV', async () => {
    await expect(compiler('./data/item/image-mapping/invalid-item-with-linked-pagination.json', rules)).rejects
        .toThrowError(/Problem parsing CSV. Each row should contain <url>|<label>/);
});
