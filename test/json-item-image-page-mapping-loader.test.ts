...
test('test loading CSV file and generating page mapped item JSON', async () => {
    ...
    expect(JSON.parse(module.source)).toEqual(JSON.parse(dataOutput));
...
test('test missing option imageServerPath', async () => {
    ...
    await expect(compiler('./data/item/image-mapping/item-with-linked-pagination.json', invalidRules)).rejects
        .toMatch(/You need to set the imageServerPath and imageType parameters in your loader options./);
...
test('test invalid CSV', async () => {
    ...
    await expect(compiler('./data/item/image-mapping/invalid-item-with-linked-pagination.json', rules)).rejects
        .toMatch(/Problem parsing CSV. Each row shouldcontain <url>|<label>/);
