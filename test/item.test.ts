...
test.each(
    ...
    "parseItemJson() parses valid item %s and returns its JSON representation",
    async (itemPath) => {
        let json = (
...
test.each(getSchemaData("cudl-schema-package-json").item.invalidTestCases)(
    "parseItemJson() rejects invalid item described by %s",
    async (testcasePath) => {
...
test("NamespaceLoader loads undefined as empty namespace", async () => {
    expect(await createNamespaceLoader().loadNamespace(undefined)).toEqual(
        Namespace.fromNamespaceMap({})
    );
    expect.assertions(1);
});
...
test("NamespaceLoader loads inline namespace", async () => {
    expect(
        await createNamespaceLoader().loadNamespace({
...
test("NamespaceLoader loads referenced namespace", async () => {
    expect(
        await createNamespaceLoader().loadNamespace("./namespace.json")
    ).toEqual(Namespace.fromNamespaceMap({ foo: "http://example.com/" }));
    expect.assertions(2);
});
