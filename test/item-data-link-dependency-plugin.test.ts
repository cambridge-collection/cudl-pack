...
test("item data links are handled specially by ItemDataLinkDependencyPlugin", async () => {
            ...
            use: [
                {
                    ...
                    options: {
                        plugins: [
                            new ItemDataLinkDependencyPlugin({
                                roles: [CDLRole.curie.uri("foo")],
                                requestQuery: "a",
                            }),
                        ],
                    },
    ...
    expect(JSON.parse(getModuleSource(item, stats))).toEqual(
        JSON.parse(await readPathAsString(item))
    );
    expect(JSON.parse(getModuleSource(`${dir}/data.json?a`, stats))).toEqual({
        handledBy: "a",
        data: { data: 42 },
    });
    expect(JSON.parse(getModuleSource(`${dir}/data.json?b`, stats))).toEqual({
        handledBy: "b",
        data: { data: 42 },
    });
});
