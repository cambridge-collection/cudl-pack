...
test.each(getSchemaData().collection.validTestCases)(
...
test.each(getSchemaData().collection.invalidTestCases)
('parseCollectionJson() rejects invalid collection described by %s', async (testcasePath) => {
    ...
    expect(() => parseCollectionJson(JSON.stringify(invalidCollection))).toThrowError(`\
