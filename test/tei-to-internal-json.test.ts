...
test("msTeiPreFilter converts full item TEI to required XML format", async () => {
    ...
    const data: string = await readPathAsString(
        "./data/tei/tei-prefiltered-item.xml"
    );
    expect(
        JSON.parse(getModuleSource("./data/tei/tei-full-item.xml", stats))
    ).toEqualXML(data);
});
...
test("jsonDocFomatter converts full item XML to internal JSON format", async () => {
    ...
    const data: string = await readPathAsString(
        "./data/tei/tei-json-output.json"
    );
    expect(
        JSON.parse(
            JSON.parse(
                getModuleSource("./data/tei/tei-prefiltered-item.xml", stats)
            )
        )
...
test("msTeiPreFilter converts small item TEI to required XML format", async () => {
    ...
    const data: string = await readPathAsString(
        "./data/tei/tei-small-prefiltered-item.xml"
    );
    expect(
        JSON.parse(getModuleSource("./data/tei/tei-small-item.xml", stats))
    ).toEqualXML(data);
});
...
test("jsonDocFomatter converts small item XML to internal JSON format", async () => {
    ...
    const data: string = await readPathAsString(
        "./data/tei/tei-small-json-output.json"
    );
    expect(
        JSON.parse(
            JSON.parse(
