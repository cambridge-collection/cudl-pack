...
test("itemPagesToInternalItemPages without resource handlers produces minimal page", async () => {
    ...
    expect(
        await itemPagesToInternalItemPages(pages, namespace(), defaultHooks())
    ).toEqual(internalPages);
});
...
test("itemPagesToInternalItemPages uses result of applying resource handlers", async () => {
    ...
    expect(
        await itemPagesToInternalItemPages(pages, namespace(), hooks)
    ).toEqual(internalPages);
});
...
test("createLogicalStructures on item with no pages results in no structures", () => {
    ...
    expect(
        createLogicalStructures(descriptions, pages, defaultHooks())
    ).toEqual([]);
});
...
test("createLogicalStructures uses descriptionTitle hook to label nodes", () => {
    ...
    const logicalStructures = createLogicalStructures(
        descriptions,
        pages,
        hooks
    );
...
Invalid description coverage: /descriptions/main/coverage/lastPage references a page that doesn\'t exist: broken`,
    ...
    [
        ...
        { firstPage: "missing", lastPage: "broken" },
        /references a page that doesn't exist/,
    ],
])(
    "createLogicalStructures rejects descriptions with coverage ranges which %s",
    (_, coverage, errorDesc) => {
        const pages: ItemPages = {
            a: { label: "p1" },
            b: { label: "p2" },
            c: { label: "p3" },
        ...
        const descriptions: ItemDescriptions = {
            main: { coverage: coverage as PageRange },
        };

        expect(() =>
            createLogicalStructures(descriptions, pages, defaultHooks())
        ).toThrowError(errorDesc as RegExp);
    }
);
        ...
        createDescriptiveMetadataSection(
            { ID: "main" },
            {
                ...
                colours: {
                    display: true,
                    label: "Colours",
                    value: [{ displayForm: "Red" }, { displayForm: "Green" }],
                    seq: 1,
                },
            }
        ),
...
test("ItemToInternalItemConverter.convert without any plugins converts items to no-frills internal items", async () => {
    const packageItem = validateItem(
        json5.parse(
    ...
    const expectedInternalItem = validateInternalItem(
        json5.parse(
            await readPathAsString(
    ...
    const convertedItem = await new ItemToInternalItemConverter().convert(
        packageItem
    );
...
test("ItemToInternalItemConverter.convert with default plugins converts items to fairly basic internal items", async () => {
    const packageItem = validateItem(
        json5.parse(
            await readPathAsString(
                "convert/item/to/internal-item/data/package-item.json5"
    ...
    const convertedItem =
        await ItemToInternalItemConverter.withDefaultPlugins().convert(
            packageItem
        );
