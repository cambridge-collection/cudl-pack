...
import {
    generateInternalItemJson,
    parseInternalItemJson,
} from "../src/internal-item";
...
test.each(getSchemaData("cudl-schema-internal-json").item.validTestCases)(
    "parseInternalItemJson() parses valid internal item %s and returns its JSON representation",
    async (itemPath) => {
        let json = (
            await readPathAsString(require.resolve(itemPath))
        ).toString();
...
test.each(getSchemaData("cudl-schema-internal-json").item.invalidTestCases)(
    "parseInternalItemJson() rejects invalid internal item described by %s",
    async (testcasePath) => {
        ...
        expect(() => parseInternalItemJson(JSON.stringify(invalidInternalItem)))
            .toThrowError(`\
...
options: $options`,
    ({ options, shouldFail }) => {
        ...
        if (shouldFail) expect(generate).toThrowError();
        else expect(typeof generate()).toBe("string");
    }
);
