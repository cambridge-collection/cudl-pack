...
test.each([
    ...
    "parseDlDatasetXml() on %s should raise an error about invalid XML",
    async (badXmlFile) => {
        const xml = await readPath("./data", badXmlFile);

        await expect(parseDlDatasetXml(xml)).rejects.toThrowError(
            /^Parsed dl-dataset XML is invalid: /
        );
    }
);

test("parseDlDatasetJson()", async () => {
    const json = await readPathAsString(
        require.resolve(
...
test.each(
    getSchemaData("cudl-schema-package-json")["dl-dataset"].validTestCases
)(
    "parseDlDatasetJson() parses valid dl-dataset %s and returns its JSON representation",
    async (dlDatasetPath) => {
        const json = (
            await readPathAsString(require.resolve(dlDatasetPath))
        ).toString();
...
test.each(
    getSchemaData("cudl-schema-package-json")["dl-dataset"].invalidTestCases
)(
    "parseDlDatasetJson() rejects invalid dl-dataset described by %s",
    async (testcasePath) => {
        ...
        expect(() => parseDlDatasetJson(JSON.stringify(invalidDlDataset)))
            .toThrowError(`\
