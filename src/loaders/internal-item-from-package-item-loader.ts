...
import {
    isItemToInternalItemConverter,
    ItemToInternalItemConverter,
} from "../convert/item/to/internal-item";
...
function getOptions(
    ...
    if (
        !(converter === undefined || isItemToInternalItemConverter(converter))
    ) {
        throw new Error(
            "converter option must be a ItemToInternalItemConverter object"
        );
    }
    ...
    return {
        ...
        converter:
            converter || ItemToInternalItemConverter.withDefaultPlugins(),
        ...objectFromEntries(
...
export default createAsyncLoader(async function (this, source) {
    ...
    return generateInternalItemJson(internalItem, {
        validate: options.postValidate,
    });
});
