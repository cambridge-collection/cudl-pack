...
abstract class BasePlugin {
    ...
    public apply(
        target: ItemToInternalItemConverter | ItemToInternalItemConversionHooks,
        item?: Item
    ): void {
        if (target instanceof ItemToInternalItemConverter) {
            target.hooks.conversion.tap(
                this.pluginName,
                this.applyHooks.bind(this)
            );
        } else {
...
The InlineNamespacePlugin doesn't support resolving namespace references: ${item["@namespace"]}`);
    ...
    protected applyHooks(
        ...
        hooks.createNamespace.tapPromise(
            this.pluginName,
            this.handleCreateNamespace.bind(this, item)
        );
    }
}
...
export class IIIFPageResourcePlugin extends ContextFreeBasePlugin {
    ...
    public async handleIIIFImagePageResource(
        convertedPage: InternalPage,
        { resource, namespace }: PageResourceContext
    ): Promise<InternalPage> {
        if (!isImageItemResource(resource, namespace)) {
            return convertedPage;
        }
        if (resource.imageType !== "iiif") {
            return convertedPage;
        }
    ...
    protected applyHooks(hooks: ItemToInternalItemConversionHooks): void {
        hooks.pageResource
            .for(PackageItemPage.image)
            .tapPromise(this.pluginName, this.handleIIIFImagePageResource);
    }
}
